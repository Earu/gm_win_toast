...
LUA_FUNCTION(show_toast)
{
	...
	if (has_image_path) 
	{
		...
		std::wstring full_path = gmod_path + L"/garrysmod/data/" + to_utf16(data_path);
		templ.setImagePath(full_path);
	}

	bool failed = WinToastLib::WinToast::instance()->showToast(templ, new WinNotifsHandler()) < 0;
	LUA->PushBool(!failed);
	return 1;
}
...
GMOD_MODULE_OPEN()
{
	...
	{
		std::string err_msg = "[WinToasts] Could not initialize: " + get_toast_error(err);
